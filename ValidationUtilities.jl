...
function validate(snapshots,valdata::IOData; prefilter::Union{NamedTuple,Nothing}=(fc_low=1/10,fc_high=1/2,order=6),
    ...
    @sync @distributed for i=1:length(snapshots)
        ...
        # Validation data
        # In case we learn timescales
        # V̂,X̂ = net(RNNvaldata)[1:2]
        # Otherwise
...
function plotValLosses(valDict::Dict;   title::String="",
                                        ...
                                        trainingYLims=:auto)
    # Initialize plots
    pltTrain,pltBest,pltTimes = plot(),plot(),plot()
    pltVal = (overlay ? plot() : [plot() for _=1:length(valDict)])
    ...
    for (i,hp) in enumerate(sort(collect(keys(valDict))))
        ...
        trainingTicks = (epochs[initEpochIndex:deltaEpochIndex:end],[string("Epoch ",epochs[i]) for i=initEpochIndex:deltaEpochIndex:length(epochs)])
        # timeTicks = (epochs[initEpochIndex:deltaEpochIndex:end],[string(getTime(valDict[hp][:times][i])) for i=initEpochIndex:deltaEpochIndex:length(epochs)])
        if epochTimes
            ...
            tTicks =  trainingTicks
        end

        # Training loss plot
        plot!(pltTrain,epochs,log10.(valDict[hp][:trainLoss]);
                                    title=title,
        ...
        commonOpts = (xticks=(overlay ? tTicks : ([tTicks[1][end],],[tTicks[2][end],])),legend=false,linewidth=linewidth,opacity=opct,color=palette(:default)[i],yticks=valYTicks)

        # Validation plots
        plt = overlay ? pltVal : pltVal[i]
        if metric == :AngSep
            plot!(plt,epochs,angSep;
                            ylabel="validation \n angular separation",
                            ylims=(0.0,1.0),
                            widen=true,
                            commonOpts...)
    ...
    commonOpts = (xlabel=hpName,marker=:circle,legend=false,color=:black)
    if plotBest
        if metric == :AngSep
            ...
            # Best results
            plot!(pltBest,sort(collect(keys(valDict))),best_angsep;
                                            xticks=collect(keys(valDict)),
                                            title="Best model angular separation",
                                            commonOpts...)
            # Corresponding times
            plot!(pltTimes,sort(collect(keys(valDict))),best_times;
            ...
            # Best results
            plot!(pltBest,sort(collect(keys(valDict))),best_losses;
    ...
    pltVal = (overlay ? pltVal : plot(pltVal...,ylabel="")) #,layout=(length(pltVal),1))
    l = @layout [a{0.4h}; b{0.6h}]
    plt = plot(pltTrain,pltVal,layout=l,size=plotSize,ylabelfontsize=ylabelfontsize,leftmargin=5Plots.mm,bottommargin=5Plots.mm)

    display(plt)
    return bestHP, [pltTrain, pltVal, pltBest, pltTimes]
end
...
function plotValVoltages(val::NamedTuple; 
    ...
    for n=1:n_neurons
        ...
        plt2 = plot!(plt2,t,val[predictionMetric][1][n][inds],
            ...
            ylabel=(isa(vLabel,Bool) ? (vLabel ? L"$\hat{v}_%$n$ [mV]" : "") : vLabel[n]),
            color=predictionColor,
            legend=(n==1 ? legend : false);
            commonOpts...)
        overlay ? push!(plts,plt2) : append!(plts,[plt1,plt2])
        if filtered
            plt3=plot(t,val[:data][2][n][inds],label=string("Data ",n),ylabel="Filtered traces",color=:black)
            push!(plts,plot(plt3,t,val[predictionMetric][2][n][inds],label=string("Filtered ",n),color=predictionColor,legend=false))
        end
...
function plotSSActivations(net::Network; n = 1,                # neuron number
    ...
    for j=1:length(ssAct[n,m])
        ...
        if !isnothing(ssAct[n,m][j][1])
            for k=1:size(ssAct[n,m][j][1],1)
                plt = plot!(plt,V̄,ssAct[n,m][j][1][k,:],title=currentNames[j],label="activation $k")
            end
        end
        if !isnothing(ssAct[n,m][j][2])
            for k=1:size(ssAct[n,m][j][2],1)
                plt = plot!(plt,V̄,ssAct[n,m][j][2][k,:],title=currentNames[j],label="inactivation $k")
            end
        end
    ...
    plot(plts...,layout=(length(plts),1),size=plotSize,ylims=actLims)
end
...
function getVoltagePlots(net::Network,data::IOData; neurons::AbstractVector{Int}=[1,],
    ...
    for n in neurons
        plt1 = plot(t[inds]/1000,V[n][inds],xlabel=tlabel,
                                            ...
                                            title=vtitle)       
        if overlay
            plt2 = plot(t[inds]/1000,V[n][inds],xlabel=tlabel,color=:black,linewidth=linewidth+1,opacity=0.3,ylabel="[mV]",label=L"$v_{%$n}$")
        else
            plt2 = plot()
        end 
        plt2 = plot!(plt2,t[inds]/1000,V̂[n][inds],xlabel="",
                                            xticks=tticks,
                                            yticks=vticks,
                                            ylims=vlims,
                                            linewidth=linewidth,
    ...
    for n=neurons
        if overlay 
            if plotIapp
                ...
                push!(plts,plot(plt_neurons[n].pltV̂,plt_neurons[n].pltIapp,layout=l,size=plotSize,legend=true))
            else
                push!(plts,plot(plt_neurons[n].pltV̂,size=plotSize,legend=true))
            end
        else
            if plotIapp
                l = @layout [a{0.425h}; b{0.425h}; c{0.15h}]
                push!(plts,plot(plt_neurons[n].pltV,plt_neurons[n].pltV̂,plt_neurons[n].pltIapp,layout=l,size=plotSize,legend=false))
            else 
                l = (2,1)
                push!(plts,plot(plt_neurons[n].pltV,plt_neurons[n].pltV̂,size=plotSize,legend=false,layout=l))
            end
        end
    end
    plot(plts...,layout=(layout== :vertical ? (length(plts),1) : (1,length(plts))),leftmargin=5Plots.mm,bottommargin=5Plots.mm)
end
...
function plotCurrents(net::Network,iodata::IOData;  n=1,    #neuron number
    ...
    for i=1:length(currentNames)
        ...
        if trueData
            ytrue = T
            n != m ? k = n : k = i  # a quick fix to plot HH and HCO results correctly
            plot!(plt,t[inds]/1000,ytrue[k,inds],
                    color=:black,
                    opacity=0.5,
                    style=:dash,
                    linewidth=linewidth)
        ...
        plt = plot!(plt,t[inds]/1000,y.+offset,
                    color=(isnothing(currentColor) ? predictionColor : currentColor),
                    linewidth=linewidth,
...
"""
function multiple_shooting_plot(net::Network,d::MSData,shotIndex::Tuple{Int64, Int64};neuron_index=1,plotStates=false,tUnit=:index,trajColor=palette(:default)[2],icColor=palette(:default)[1],icSize=5,dataColor=:black)
    Ni,Nf = shotIndex
    V̂seq,X̂seq = net(d)
    V̂,X̂ = shotTrajectories(V̂seq,X̂seq)
    ...
    plt=plot(reduce(vcat,tinds)*dt,d.rawdata[1].V[neuron_index][reduce(vcat,tinds).+ 1],xlabel=tLabel,color=dataColor,label="v_t")
    if plotStates
        pltX=[plot(xlabel=tLabel,color=dataColor) for j=1:size(X̂[1][neuron_index][neuron_index],1)]
    end
    
    # Plot shot trajectories
    for (i,n) in enumerate(Ni:Nf)
        plt = plot!(plt,tinds[i]*dt,vec(V̂[n][neuron_index]),color=trajColor,label= (i==1 ? "vcheck_t" : false))
        if plotStates
            for j=1:size(X̂[1][neuron_index][neuron_index],1)
                pltX[j] = plot!(pltX[j],tinds[i]*dt,X̂[n][neuron_index][neuron_index][j,:],color=trajColor,label= (i==1 ? "vcheck_t" : false))
            end
        end
    end
    
    # Plot initial conditions
    plt=scatter!(plt,binds*dt,d.V₀[neuron_index].value[1,Ni:Nf],color=icColor,markersize=icSize,markerstrokewidth=2,label="Initial conditions")
    if plotStates
        for j=1:size(X̂[1][neuron_index][neuron_index],1)
            pltX[j]=scatter!(pltX[j],binds*dt,d.X₀[neuron_index,neuron_index].value[j,Ni:Nf],color=icColor,markerstrokewidth=2,markersize=icSize,label="Initial conditions")
        end
        return plt,pltX
    else
        return plt
